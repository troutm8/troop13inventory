...
describe('addItem', () => {
  it('adds an item with correct defaults', () => {
    const { result } = renderInventory();

    let newItem;
    act(() => {
      newItem = result.current.addItem(sampleItem);
    });
  ...
  it('appears in the items list after adding', () => {
    const { result } = renderInventory();
  ...
  it('can add multiple items', () => {
    ...
    act(() => {
      result.current.addItem(sampleItem);
      result.current.addItem({ ...sampleItem, name: 'Camp Stove', category: 'Cooking' });
    });
  ...
  it('generates unique IDs for each item', () => {
    ...
    let item1, item2;
    act(() => {
      item1 = result.current.addItem(sampleItem);
      item2 = result.current.addItem({ ...sampleItem, name: 'Camp Stove' });
    });
...
describe('updateItem', () => {
  it('updates item fields', () => {
    ...
    let item;
    act(() => {
      item = result.current.addItem(sampleItem);
    });
    act(() => {
      result.current.updateItem(item.id, { name: 'Updated Tent', quantity: 5 });
    });
  ...
  it('logs an activity entry for the update', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      result.current.updateItem(item.id, { condition: 'Fair' });
    });
  ...
  it('does not affect other items', () => {
    ...
    act(() => {
      result.current.updateItem(item1.id, { name: 'Big Tent' });
    });
...
describe('deleteItem', () => {
  ...
  it('cleans up related checkouts when item is deleted', () => {
    const { result } = renderInventory();

    let item, trip;
    act(() => {
      item = result.current.addItem(sampleItem);
      trip = result.current.addTrip(sampleTrip);
    });
    act(() => {
      result.current.checkoutItem(item.id, trip.id, 1, 'John');
    });
    act(() => {
      result.current.deleteItem(item.id);
    });
  ...
  it('logs an activity entry for deletion', () => {
    const { result } = renderInventory();
...
describe('addTrip', () => {
  it('creates a trip with correct defaults', () => {
    const { result } = renderInventory();
  ...
  it('appears in the trips list', () => {
    const { result } = renderInventory();

    act(() => {
      result.current.addTrip(sampleTrip);
    });
...
describe('updateTrip', () => {
  it('updates trip status', () => {
    ...
    act(() => {
      result.current.updateTrip(trip.id, { status: 'Active' });
    });
...
describe('checkoutItem', () => {
  it('creates a checkout record with correct fields', () => {
    const { result } = renderInventory();
    ...
    let checkout;
    act(() => {
      checkout = result.current.checkoutItem(item.id, trip.id, 2, 'Mike');
    });
  ...
  it('changes item status to Checked Out', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      result.current.checkoutItem(item.id, trip.id, 1, 'Mike');
    });
  ...
  it('adds the checkout to the checkouts list', () => {
    const { result } = renderInventory();
...
describe('returnItem', () => {
  it('records return details on the checkout', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      checkout = result.current.checkoutItem(item.id, trip.id, 1, 'Mike');
    });
    act(() => {
      result.current.returnItem(checkout.id, 'Sarah', 'Good', 'All clean');
    });
  ...
  it('sets item status back to In Storage when returned in good condition', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      result.current.returnItem(checkout.id, 'Sarah', 'Good', '');
    });
  ...
  it('sets item status to Damaged when returned with Needs Repair', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      result.current.returnItem(checkout.id, 'Sarah', 'Needs Repair', 'Pole is bent');
    });
  ...
  it('logs a return activity', () => {
    ...
    act(() => {
      result.current.returnItem(checkout.id, 'Sarah', 'Good', 'Looks great');
    });
...
describe('getItemCheckouts', () => {
  it('returns only checkouts for the specified item', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      result.current.checkoutItem(item1.id, trip.id, 1, 'Mike');
      result.current.checkoutItem(item2.id, trip.id, 1, 'John');
    });
  ...
  it('returns checkouts sorted by most recent first', () => {
    const { result } = renderInventory();

    let item, trip1, trip2;
    act(() => {
      item = result.current.addItem(sampleItem);
      trip1 = result.current.addTrip({ ...sampleTrip, name: 'Trip 1' });
      trip2 = result.current.addTrip({ ...sampleTrip, name: 'Trip 2' });
    });
    act(() => {
      result.current.checkoutItem(item.id, trip1.id, 1, 'Mike');
    });
    // Return first so we can check out again
    act(() => {
      const co = result.current.checkouts[0];
      result.current.returnItem(co.id, 'Mike', 'Good', '');
    });
    act(() => {
      result.current.checkoutItem(item.id, trip2.id, 1, 'John');
    });
  ...
  it('returns empty array for item with no checkouts', () => {
    const { result } = renderInventory();
...
describe('getTripCheckouts', () => {
  it('returns only checkouts for the specified trip', () => {
    const { result } = renderInventory();
...
describe('getAvailableItems', () => {
  it('returns only items with In Storage status', () => {
    const { result } = renderInventory();
  ...
  it('returns all items when none are checked out', () => {
    const { result } = renderInventory();

    act(() => {
      result.current.addItem(sampleItem);
      result.current.addItem({ ...sampleItem, name: 'Camp Stove' });
    });

    expect(result.current.getAvailableItems()).toHaveLength(2);
  });

  it('returns empty array when all items are checked out', () => {
    const { result } = renderInventory();
...
describe('stats', () => {
  ...
  it('reflects correct counts after adding items', () => {
    const { result } = renderInventory();
  ...
  it('updates when items are checked out', () => {
    const { result } = renderInventory();
  ...
  it('counts damaged items separately', () => {
    const { result } = renderInventory();
    ...
    act(() => {
      result.current.returnItem(checkout.id, 'Sarah', 'Needs Repair', 'Broken pole');
    });
  ...
  it('counts upcoming trips', () => {
    const { result } = renderInventory();

    act(() => {
      result.current.addTrip(sampleTrip);
      result.current.addTrip({ ...sampleTrip, name: 'Fall Campout' });
    });
...
describe('localStorage persistence', () => {
  ...
  it('persists checkouts to localStorage', () => {
    const { result } = renderInventory();
